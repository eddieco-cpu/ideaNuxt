        ...
        >
            ...
            <article class="w-[436px] max-xl:w-auto">
                ...
                <div class="flex justify-between items-end mb-2">
                    <h2 class="text-3xl font-bold max-md:text-xl">$ {{ pageData.price }}</h2>
                    <p class="text-Neutral-600-Dark-Primary text-xs">
                        目標金額
                        <span>$ {{ pageData.goal_price }}</span>
                    </p>
                </div>
                ...
                <div class="flex items-center gap-3 mt-1 mb-1">
                    <UMeter
                        :value="pageData.achievementRate"
                        :ui="{ meter: { color: 'text-Primary-500-Primary', background: 'bg-Primary-50' } }"
                        color="violet"
                    />
                    <span class="text-Primary-500-Primary font-medium text-xs">{{ pageData.achievementRate }}%</span>
                </div>

                <div class="flex justify-between items-center mb-6 max-md:block">
                    <p class="text-xs text-Neutral-600-Dark-Primary">
                        募資時間 <span>{{ pageData.start_time }}</span> - <span>{{ pageData.end_time }}</span>
                    </p>
                    <div class="inline-flex justify-between items-center max-md:mt-2">
                        <p class="mr-3 ring-1 ring-Primary-200 rounded flex justify-center items-center p-1">
                            ...
                            <span class="ml-1 text-Primary-600-Dark-Primary text-sm">{{ pageData.dataTime }}日</span>
                        </p>
                        <p class="ring-1 ring-Primary-200 rounded flex justify-center items-center p-1">
                            ...
                            <span class="ml-1 text-Primary-600-Dark-Primary text-sm">{{ pageData.people }}人</span>
                        </p>
                    </div>
                </div>
        ...
        >
            ...
            >
                ...
                <template v-if="activeNavItemId === 'c'">
                    <article class="bg-white p-6 rounded-lg">
                        <UiAccordion :items="pageData.questions" />
                    </article>
                </template>
            </section>
...
const productsWithSoldOut = computed(() =>
   cards.value.map(card => ({
    ...card,
    soldOut: card.sell_qty >= card.limit_qty,
    stock_left: card.limit_qty - card.sell_qty,
    save_price: card.original_price - card.sell_price
